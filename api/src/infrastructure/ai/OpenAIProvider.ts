...
export class OpenAIProvider extends BaseAIProvider {
  ...
  async generateResponse(
    ...
    try {
      // Select model: gpt-5.1-chat-latest for Haiku (faster), gpt-5.1 for Sonnet (reasoning)
      const selectedModel = useHaiku ? "gpt-5.1-chat-latest" : "gpt-5.1";
...
IMPORTANT: When the request mentions "this image" or "these images", use the EXACT URLs listed above. DO NOT use stock photos or other URLs.`;
      ...
      // Build user message content with images if provided (OpenAI format)
      ...
      const stream = await this.client.chat.completions.create({
        ...
        tools,
        stream: true,
        stream_options: { include_usage: true } // Critical for token tracking in streaming
      });
      ...
      for await (const chunk of stream) {
        ...
        // Usage data comes in final chunks when stream_options is set
        if (chunk.usage) {
      ...
      console.log("OpenAI API streaming completed");
      ...
      try {
        ...
        console.log(`[OpenAIProvider] Logged AI execution to ai_logs table (model: ${model}, cost: $${cost.toFixed(6)})`);
      } catch (error) {
        console.error(`[OpenAIProvider] CRITICAL: Failed to log to ai_logs table:`, error);
