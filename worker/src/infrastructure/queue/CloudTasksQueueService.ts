...
export class CloudTasksQueueService implements IQueueService {
  ...
  constructor() {
    ...
    console.log(`[Worker] CloudTasksQueueService initialized with queue: ${this.queuePath}, worker: ${this.workerUrl}`);
  }
  ...
  async sendMessage(message: QueueMessage): Promise<void> {
    ...
    if (isHttps) {
      ...
      console.log('[Worker CloudTasks] Skipping OIDC token for HTTP URL (local dev)');
    }
    ...
    console.log(`[Worker CloudTasks] Created task: ${response.name}`);
  }
  ...
  async deleteMessage(_receiptHandle: string): Promise<void> {
    ...
    console.log('[Worker CloudTasks] deleteMessage called - no-op in push model (task auto-deleted on success)');
  }
}
