...
export default {
  async fetch(request, env) {
    ...
    // For custom hostnames routed through Cloudflare for SaaS with fallback origin,
    // the hostname may be mangled (e.g., "www.example.com.dev.huskystudio.app")
    let hostname = url.hostname;

    // If the hostname looks like it's been mangled, extract the original custom domain
    if (hostname.includes('.dev.huskystudio.app') && hostname !== url.hostname.replace(/\.dev\.huskystudio\.app$/, '')) {
      const parts = hostname.split('.dev.huskystudio.app');
      if (parts[0] && parts[0].includes('.')) {
        // This is a mangled custom domain like "www.interviewtime.ai.dev.huskystudio.app"
        const originalHostname = hostname;
        hostname = parts[0]; // Extract "www.interviewtime.ai"
        console.log(`[Worker] Detected mangled hostname: ${originalHostname} → extracted: ${hostname}`);
      }
    }
    ...
    if (!projectId) {
      console.log(`[Worker] ❌ ERROR: No project found for hostname: ${hostname}`);
